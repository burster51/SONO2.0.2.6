...
unsigned int nTargetSpacing3    = 120;              // PoS and PoW together sould get the chain to 60 sec
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    else if (pindexBest->nHeight <= PoWReduction1)
        nSubsidy = 2 * COIN;
    else if (pindexBest->nHeight <= PoWReduction2)
        nSubsidy = 1.8 * COIN;
    else if (pindexBest->nHeight <= PoWReduction3)
        nSubsidy = 1.6 * COIN;
    else if (pindexBest->nHeight <= PoWReduction4)
        nSubsidy = 1.4 * COIN;
    else if (pindexBest->nHeight <= PoWReduction5)
        nSubsidy = 1.2 * COIN;
    else
        nSubsidy = 1 * COIN;
    }
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    else if (pindexBest->nHeight <= PoSRewardReduction1)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD / 365;
    else if (pindexBest->nHeight <= PoSRewardReduction2)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD1 / 365;
    else if (pindexBest->nHeight <= PoSRewardReduction3)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD2 / 365;
    else if (pindexBest->nHeight <= PoSRewardReduction4)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD3 / 365;
    else if (pindexBest->nHeight <= PoSRewardReduction5)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD4 / 365;
    else
        nSubsidy = nCoinAge * COIN_YEAR_REWARD5 / 365;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    */
    ...
    //    printf("XXXXXX nValueOut %" PRId64 " nValueIn %" PRId64 "\n",nValueOut,nValueIn);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version")
    {
        ...
        // Disconnect nodes with old versions starting at block height BlocktimeOneMinHeight
        if (nBestHeight >= BlocktimeOneMinHeight && pfrom->nVersion < KICK_BELOW_THIS_VERSION)
            oldVersion = true;
